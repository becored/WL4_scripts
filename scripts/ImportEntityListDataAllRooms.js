function ImportEntityListDataAllRooms()
{
    ...
    for (current_difficulty = 0; current_difficulty < 3; current_difficulty ++) {
        for (current_room_id = 0; current_room_id < interface.GetRoomNum(); current_room_id ++) {
            ...
            interface.SetEntityListData(interface.ReadTxtFile(output_dir + "Room" + ("00" + interface.GetCurRoomId()).slice(-2) + "_" + difNames[current_difficulty] + ".bin"), current_difficulty);
            interface.log("File imported: " + output_dir + "Room" + ("00" + interface.GetCurRoomId()).slice(-2) + "_" + difNames[current_difficulty] + ".bin");
        }
    }
}
